...
export default function Hero({ claims = [] }) {
  const ref = useRef(null)
  const reduce = useReducedMotion()
  const { scrollYProgress } = useScroll({ target: ref, offset: ['start start', 'end start'] })

  // The footage drifts slower than the page, so the type feels like it sits in front.
  // The drift (max 6%) stays inside the scale overflow (8% per edge at 1.16), so the
  // frame never pulls away from an edge and exposes the ground behind it.
  ...
  return (
    <section className="hero" ref={ref} id="top">
      ...
      {/* The ribbon rides the bottom of the same 90vh block. No fill above it, so the
          footage runs unbroken down to the lettering; the cream fill below hands off
          to the next section without a seam. */}
      <Marquee
